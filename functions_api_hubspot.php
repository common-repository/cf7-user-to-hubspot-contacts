...
class WM_CF7USRTOHS{
	...
	public static function wm_get_key(){
		return get_option('hubspot_key');
	}
	
	
	public static function wm_update_key( &$hubspot_key='' ){
		$hubspot_key = substr($hubspot_key, 0, 100);
		$hubspot_key = stripslashes( $hubspot_key );
		//$hubspot_key = sanitize_key( $mailchimp_key );
		$hubspot_key = sanitize_text_field( $mailchimp_key );
		update_option('hubspot_key', $hubspot_key, false);
	}
}